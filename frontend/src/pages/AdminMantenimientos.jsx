...
import { FaSearch, FaTimes, FaEdit, FaWrench, FaWhatsapp, FaPhone } from 'react-icons/fa'
    ...
    <div>
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-green-500 rounded-xl flex items-center justify-center">
            <FaWrench size={16} className="text-white" />
          </div>
          <div>
            <p className="text-sm text-gray-400">{mantenimientos.length} mantenimientos registrados</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-xl font-black text-gray-900">{mantenimientos.length}</p>
            <p className="text-xs text-gray-400">Total</p>
          </div>
        </div>
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex items-center gap-3">
          <div className="w-10 h-10 bg-yellow-100 rounded-xl flex items-center justify-center">
            <FaWrench size={16} className="text-yellow-600" />
          </div>
          <div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex items-center gap-3">
          <div className="w-10 h-10 bg-blue-100 rounded-xl flex items-center justify-center">
            <FaWrench size={16} className="text-blue-600" />
          </div>
          <div>
      ...
      {cargando ? (
        ...
        <div className="text-center py-20 bg-white rounded-2xl border border-gray-100">
          <FaWrench size={36} className="text-gray-300 mx-auto mb-3" />
          <p className="text-gray-500 font-medium">No se encontraron mantenimientos</p>
            ...
            <tbody className="divide-y divide-gray-50">
              {mantenimientosFiltrados.map((m) => (
                <tr key={m.id} className="hover:bg-gray-50 transition">
                  <td className="px-5 py-4 font-bold text-green-600">{m.codigo}</td>
                  <td className="px-5 py-4 font-medium text-gray-900">{m.nombre_cliente}</td>
                  <td className="px-5 py-4 text-gray-500">{m.telefono}</td>
                  <td className="px-5 py-4">
                    <span className={`px-2.5 py-1 rounded-full text-xs font-bold capitalize ${tipoColor[m.tipo] || 'bg-gray-100 text-gray-700'}`}>
                      {m.tipo}
                    </span>
                  </td>
                  <td className="px-5 py-4 text-gray-500">{m.distrito}</td>
                  <td className="px-5 py-4">
                    <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${estadoColor[m.estado] || 'bg-gray-100 text-gray-700'}`}>
            ...
            <div className="px-6 py-5 border-b border-gray-100 flex justify-between items-center">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">Detalle del mantenimiento</p>
                </div>
              </div>
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-gray-50 rounded-xl p-3">
                  ...
                  <p className="text-sm font-bold text-gray-900">{seleccionado.nombre_cliente}</p>
                </div>
                <div className="bg-gray-50 rounded-xl p-3">
                  <p className="text-xs text-gray-400 mb-0.5">Telefono</p>
                  <p className="text-sm font-bold text-gray-900">{seleccionado.telefono}</p>
                </div>
                <div className="bg-gray-50 rounded-xl p-3">
                  <p className="text-xs text-gray-400 mb-0.5">Correo</p>
                  <p className="text-sm font-bold text-gray-900">{seleccionado.correo}</p>
                </div>
                <div className="bg-gray-50 rounded-xl p-3">
                  <p className="text-xs text-gray-400 mb-0.5">Distrito</p>
                  <p className="text-sm font-bold text-gray-900">{seleccionado.distrito}</p>
                </div>
                <div className="bg-gray-50 rounded-xl p-3">
                  <p className="text-xs text-gray-400 mb-0.5">Tipo</p>
                  <p className="text-sm font-bold text-gray-900 capitalize">{seleccionado.tipo}</p>
                </div>
                <div className="bg-gray-50 rounded-xl p-3">
                  <p className="text-xs text-gray-400 mb-0.5">Tipo de puerta</p>
                  <p className="text-sm font-bold text-gray-900">{seleccionado.tipo_puerta}</p>
                </div>
                <div className="bg-gray-50 rounded-xl p-3 col-span-2">
                  <p className="text-xs text-gray-400 mb-0.5">Disponibilidad</p>
                  <p className="text-sm font-bold text-gray-900">{seleccionado.disponibilidad}</p>
                </div>
              </div>

              {seleccionado.descripcion_problema && (
                <div className="bg-gray-50 rounded-xl p-4">
                  <p className="text-xs text-gray-400 mb-1">Descripcion del problema</p>
