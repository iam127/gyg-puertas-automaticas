    ...
    visitas: [
      {
        ...
        materiales_necesarios: 'Bisagras industriales x4, Guías de arrastre x2, Motor CAME BX-243',
        observaciones: 'El sistema batiente se encuentra desviado por choque leve de camión.',
...
export const CotizacionService = {
  getMyCotizaciones: async (tecnicoId: number): Promise<Cotizacion[]> => {
    ...
    const response = await api.get<Cotizacion[]>('/cotizaciones/mis_cotizaciones/');
  ...
  updateCotizacionStatus: async (id: number, status: any): Promise<Cotizacion> => {
    if (useAuthStore.getState().isDemoMode) {
      MOCK_COTIZACIONES = MOCK_COTIZACIONES.map((c) => {
        if (c.id === id) return { ...c, estado: status, actualizado_en: new Date().toISOString() };
        return c;
      });
    ...
    const response = await api.patch<Cotizacion>(`/cotizaciones/${id}/`, { estado: status });
    return response.data;
  },
          ...
          const updatedVisitas = c.visitas?.map((v) => {
            if (v.id === visitaId) {
              return {
                ...
                materiales_necesarios: data.materiales_necesarios,
